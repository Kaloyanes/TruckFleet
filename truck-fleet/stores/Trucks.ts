...
export const useTrucksStore = defineStore("myTrucksStore", () => {
  ...
  const filterByStatus = (status: string) => {
    ...
    trucks.value = unfilteredTrucks.value.filter(
  ...
  const filterByLicensePlate = (licensePlate: string) => {
    ...
    trucks.value = unfilteredTrucks.value.filter((truck) =>
      (truck.licensePlate as string)
        .toLowerCase()
  ...
  const updateTruckStatus = async (id: string, status: string) => {
    if (!id || !status) return;

    const db = useFirestore();
    const truckRef = doc(db, "trucks", id);
    await updateDoc(truckRef, { status });
    trucks.value = trucks.value.map((truck) => {
      if (truck.id === id) {
        return { ...truck, status };
      }
